...
Thursday afternoon at the Boston WeWork. I'd been using Claude Code's web interface to knock out GitHub issues, but the workflow was painful. Copy the issue description. Paste it into Claude. Manually shuttle context back and forth. There had to be a better way to get Claude to just *read the issue and fix it*.

I also had a security problem. Claude Code's `--dangerously-skip-permissions` flag lets the agent run wild — editing files, executing commands, doing whatever it takes to finish the job. Powerful, but I wasn't about to run that on my local machine with access to everything.
...
I wanted a tool that could take a GitHub issue number and produce a pull request — without me babysitting it, and without giving an AI agent the keys to my kingdom.
