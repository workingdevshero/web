...
It's 2026 and every assistant ships with a credit card. Subscriptions pile up, API keys rot in `.env` files, and the "free tier" turns out to cost $79/month in surprise overages. Meanwhile, a quiet standard — HTTP 402 — has finally woken up, and a growing ecosystem of APIs now accept USDC micropayments instead of keys.

This is a better deal for agents. No signup, no dashboard, no renewal email. The agent hits an endpoint, the server says "that'll be half a cent," the agent signs a USDC payment on Base, and the data flows back.
...
- [VirtualBox](https://www.virtualbox.org/) (we'll run Ubuntu 26.04 LTS in a VM) — running OpenClaw inside a VM or on a VPS (e.g. [Hostinger](https://www.hostinger.com/), [Linode](https://www.linode.com/), [DigitalOcean](https://www.digitalocean.com/)) keeps it isolated from your personal files
...
Spin up an Ubuntu 26.04 VM in VirtualBox with 8 GB RAM and 25 GB storage. Install Guest Additions if you want clipboard sharing.

We need `curl` and `Node.js`:
...
Configure npm to install global packages into your home directory so we don't need `sudo` later:
...
Pin to a known-good version since newer releases occasionally have onboarding bugs:

```bash
npm install -g openclaw@2026.4.23
openclaw --version  # OpenClaw 2026.4.23
```
...
The TUI (terminal UI) is an interactive chat you can use to talk to OpenClaw directly. When onboarding completes, a `"Wake up, my friend!"` message is sent and the agent responds. Pop over to Discord and `@YourBot hello` — same agent, different surface.
...
Now fund the wallet. Send ~$1 USDC on the **Base** network (not Ethereum mainnet — those are different chains, and USDC sent to the wrong chain is painful to recover) to the address you just generated.

Don't keep more in this wallet than you're willing to lose. Always keep small amounts in it.

## Install the ChartSplat Skill

Back in your VM. ChartSplat is a chart-generation service with an [x402 integration](https://clawhub.ai/bobbyg603/chartsplat-x402) on ClawHub (OpenClaw's skill registry). We'll have the agent install the skill itself — you don't need to manually run `npm install` or copy files anywhere.
...
We are going to set up a cron job to receive the desired information. Before going further, pre-approve this device's scopes so OpenClaw doesn't pause for permission later. Run:
...
**Onboarding wizard crashes.** Confirm you're on the pinned version:
```bash
