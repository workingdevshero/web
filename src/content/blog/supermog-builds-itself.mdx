...
In the spirit of the whole project, no video editor was opened. The build was shot in one static take on the bench at the [Sparx](https://sparx-fire.com) lab — 95 minutes across two clips (there's a mid-build run for a flathead screwdriver hiding in there). The edit is a single ffmpeg filter graph, written in conversation with Claude: both clips concatenated and run at 115×, titles drawn in the site's own Bricolage Grotesque, then a snap back to real time for the moment the finished case gets held up to the camera.

The music came out of [Venice's](https://venice.ai) music API — a 66-second instrumental prompt to the ElevenLabs Music model, third take kept. Then the fun part: a little Python onset analysis found the track's drop at 16 seconds and its strongest kicks, and the glitch bursts — hard RGB split plus analog noise — are placed on those beats, with a final burst landing exactly on the timelapse-to-real-time cut. The audio visualizer riding the bottom edge is ffmpeg's `showfreqs` in the brand palette, and the end card's background is color-sampled from the mascot PNG so the two are seamless. Total tooling: a phone, ffmpeg, one API, and a chat window.
...
We keep going back and forth on this one. Part of us wants to see what a community of agents-building-agents looks like; the other part knows exactly how much throughput this buys and what that's worth. So we're letting demand decide: if you'd want the Supermog source — to run it, to study it, or to acquire it outright — tell us. Ping [@workingdevshero](https://x.com/workingdevshero) on X or email [bobby@workingdevshero.com](mailto:bobby@workingdevshero.com). Enough interest, and we'll do something about it.
