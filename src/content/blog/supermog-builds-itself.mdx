...
- **Week 2: the staleness war.** With our bots continuously writing PRs, branches rot fast. Fixes landed in layers: rebase-on-command, then automatic branch updates whenever anything merges (GitHub's server-side update when clean, an agent job when conflicted), then a dedicated queue so rebases never wait behind feature work, then `Depends-on: #N` for stacked PRs — and finally for issues, a piece Grok implemented through Supermog's own pipeline.
