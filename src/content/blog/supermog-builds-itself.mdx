...
- **This week: the models are a dial.** Which model implements and which one reviews are settings, not architecture — changeable on a running fleet. That stopped being theoretical the day one provider started timing out mid-job: we pointed both roles at the other model, and the queue kept moving. Independent review matters more than which model is doing it.
