...
- **Week 2: it files its own bugs.** When a run hits something wrong, the agent reports it through [BugSplat](https://www.bugsplat.com)'s user feedback tool, tagged with Supermog's own trigger tag, so the bot picks its own report back up as work. The agent is both the user and the developer. The loop closed.
